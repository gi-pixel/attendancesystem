...
let marksData = [];
let studentsData = [];
let currentMarksCourse = '';

// Load Marks
document.getElementById('loadMarksBtn')?.addEventListener('click', async () => {
    const course = document.getElementById('marksCourseSelect').value;
    currentMarksCourse = course;
    
    const loadBtn = document.getElementById('loadMarksBtn');
    const loading = document.getElementById('marksLoading');
    const tableWrapper = document.getElementById('marksTableWrapper');
    const tbody = document.getElementById('marksTableBody');
    const statusDiv = document.getElementById('marksStatus');
    ...
    try {
        // Fetch all students
        const studentsResponse = await fetch('/api/get-students');
        const studentsResult = await studentsResponse.json();
        studentsData = studentsResult.students || [];
        
        // Fetch marks for this course
        const marksResponse = await fetch(`/api/get-marks?course=${course}`);
        const marksResult = await marksResponse.json();
        marksData = marksResult.records || [];
        
        // Merge data: show all students, pre-fill marks if they exist
        const mergedData = studentsData.map(student => {
            const existingMark = marksData.find(m => m.indexNumber === student.indexNumber);
            return {
                indexNumber: student.indexNumber,
                name: student.name,
                midsem: existingMark ? existingMark.midsem : '',
                classAssessment: existingMark ? existingMark.classAssessment : '',
                exam: existingMark ? existingMark.exam : ''
            };
        });
        
        if (mergedData.length === 0) {
            tbody.innerHTML = '<tr><td colspan="5" class="empty-state">No students found. Please upload a class list first.</td></tr>';
        } else {
            tbody.innerHTML = mergedData.map((student, index) => `
                <tr>
                    ...
                    <td>
                        <input type="text" 
                               ...
                               placeholder="—"
                               style="width: 80px; padding: 0.35rem 0.5rem; border: 1.5px solid var(--border-main); border-radius: var(--radius-sm); background: var(--bg-elevated); color: var(--text-primary); font-size: 0.85rem; text-align: center;">
                    </td>
                    <td>
                        <input type="text" 
                               class="mark-input" 
                               data-index="${student.indexNumber}" 
                               data-field="classAssessment" 
                               value="${student.classAssessment || ''}"
                               placeholder="—"
                               style="width: 100px; padding: 0.35rem 0.5rem; border: 1.5px solid var(--border-main); border-radius: var(--radius-sm); background: var(--bg-elevated); color: var(--text-primary); font-size: 0.85rem; text-align: center;">
                    </td>
                    <td>
                        <input type="text" 
                               class="mark-input" 
            ...
            document.querySelectorAll('.mark-input').forEach(input => {
                // Only allow numeric characters on input
                ...
                input.addEventListener('blur', function(e) {
                    const indexNumber = this.dataset.index;
                    const field = this.dataset.field;
                    const value = this.value.trim();
                    
                    // Don't save if empty
                    if (value === '') {
                        // Remove empty value from sheet (delete)
                        updateMark(indexNumber, field, value);
                        return;
                    }
                    
                    // Validate - only numbers allowed (already enforced on input)
                    if (value && !/^[\d.]+$/.test(value)) {
                        this.value = '';
                        updateMark(indexNumber, field, '');
                        showMarksStatus('Invalid characters removed. Only numbers allowed.', 'warning');
                        return;
                    }
            ...
            statusDiv.textContent = `Loaded ${mergedData.length} students. Click on a mark to edit, then click away to save.`;
        }
        ...
        tbody.innerHTML = `<tr><td colspan="5" class="empty-state">Error loading data: ${error.message}</td></tr>`;
        tableWrapper.style.display = 'block';
        statusDiv.textContent = 'Error loading data. Please try again.';
    } finally {
        loadBtn.disabled = false;
...
async function updateMark(indexNumber, field, value) {
    try {
        ...
        if (data.success) {
            const action = data.action === 'inserted' ? 'created new record for' : 'updated';
            showMarksStatus(`✅ ${action} ${field} for ${indexNumber}`, 'success');
        } else {
            showMarksStatus(`❌ Error: ${data.error || 'Failed to save mark'}`, 'error');
        }
    } catch (error) {
        console.error('Error updating mark:', error);
        showMarksStatus(`❌ Network error: ${error.message}`, 'error');
    }
}

// Show status message for marks
function showMarksStatus(message, type) {
    const statusDiv = document.getElementById('marksStatus');
    if (!statusDiv) return;
    
    statusDiv.textContent = message;
    statusDiv.style.color = type === 'success' ? 'var(--success)' : 
                            type === 'warning' ? 'var(--warning)' : 
                            'var(--danger)';
    statusDiv.style.fontWeight = '500';
    
    // Clear after 3 seconds for success/error
    if (type !== 'warning') {
        setTimeout(() => {
